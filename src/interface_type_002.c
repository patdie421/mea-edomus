...
struct callback_commissionning_data_s // donnee "userdata" pour les callbacks
{
   PyThreadState  *mainThreadState;
...
struct callback_xpl_data_s // donnee "userdata" pour les callbacks
{
   PyThreadState  *mainThreadState;
   PyThreadState  *myThreadState;
   sqlite3        *param_db;
...
int interface_type_002_commissionning_callback(int id, unsigned char *cmd, uint16_t l_cmd, void *data, char *addr_h, char *addr_l)
{
   ...
   else if(s == SQLITE_DONE) // pas de ligne trouvée
   {
      // pas de ligne trouvé
      sqlite3_finalize(stmt);
      return 0;
      ...
      if(!out_queue_elem(callback_data->queue, (void **)&e))
      {
         ...
         while(1)
         {
            ...
            if (s == SQLITE_ROW)
            {
               ...
               {
                  // traitement de l'erreur malloc √† faire
               }
            }
            else if (s == SQLITE_DONE)
            {
...
pthread_t *start_interface_type_002_thread(xbee_xd_t *xd, sqlite3 *db, tomysqldb_md_t *md,thread_f function, void *data, struct callback_data_s *callback_xbeedata)
{
   ...
   // pr√©paration de la file de traitement "iodata", son verrou et sa condition
...
int start_interface_type_002(interface_type_002_t *i002, sqlite3 *db, int id_interface, const unsigned char *dev, tomysqldb_md_t *md)
{
   ...
   // r√©cup√©ration de l'adresse de l'xbee connecter au PC (pas forcement le coordinateur).
   uint32_t addr_64_h;
   uint32_t addr_64_l;
   {
   ...
   /* a remplacer par l'appel du plugin python associÈ a l'interface s'il existe
    int xbee_flag=0;
   ...
   xbeedata_callback_params->queue=NULL;
   xbeedata_callback_params->xd=xd;
   xbeedata_callback_params->param_db=db;
   
   i002->thread_data=start_interface_type_002_thread(xd,db,md,(thread_f)_thread_interface_type_002_xbeedata,(void *)i002,xbeedata_callback_params);
   xbee_set_iodata_callback2(xd, inteface_type_002_xbeedata_callback, (void *)xbeedata_callback_params);
   xbee_set_dataflow_callback2(xd, inteface_type_002_xbeedata_callback, (void *)xbeedata_callback_params);
   ...
   xbee_set_commissionning_callback2(xd, interface_type_002_commissionning_callback, (void *)commissionning_callback_params);
...
clean_exit:
   ...
   if(xbeedata_callback_params)
   {
      if(xbeedata_callback_params->queue && xbeedata_callback_params->queue->nb_elem>0) // on vide s'il y a quelque chose avant de partir
         clear_queue(xbeedata_callback_params->queue,_iodata_free_queue_elem);
