...
int16_t sendAllxPLTrigger(interface_type_004_t *i004)
{
   ...
   while(currentLight)
   {
      ...
      char *huename = my_cJSON_GetItemByName(currentLight, "name")->valuestring;
      ...
      if(deviceName)
      {
         cJSON *stateCurrent = my_cJSON_GetItemByName(currentLight, "state");
         if(stateCurrent)
         {
            cJSON *onCurrent        = my_cJSON_GetItemByName(stateCurrent, "on");
            cJSON *reachableCurrent = my_cJSON_GetItemByName(stateCurrent, "reachable");
            int16_t state = 1;
...
int16_t whatChange(interface_type_004_t *i004)
{
   ...
   while(currentLight)
   {
      ...
      {
         cJSON *stateCurrent = my_cJSON_GetItemByName(currentLight, "state");
         cJSON *stateLast    = my_cJSON_GetItemByName(lastLight,    "state");
         if(stateCurrent && stateLast)
         {
            cJSON *onCurrent        = my_cJSON_GetItemByName(stateCurrent, "on");
            cJSON *onLast           = my_cJSON_GetItemByName(stateLast,    "on");
            cJSON *reachableCurrent = my_cJSON_GetItemByName(stateCurrent, "reachable");
            cJSON *reachableLast    = my_cJSON_GetItemByName(stateLast,    "reachable");
            int16_t newState = -1;
            if(e->reachable_use == 1)
            {
...
int load_interface_type_004(interface_type_004_t *i004, sqlite3 *db)
{
   ...
   sprintf(sql_request,"SELECT * FROM sensors_actuators WHERE id_interface=%d and sensors_actuators.state='1'", i004->id_interface);
   ...
   while (1) // boucle de traitement du résultat de la requete
   {
      ...
      if (s==SQLITE_ROW)
      {
         ...
         if(hue_params && nb_hue_params>0)
         {
            ...
            {
               VERBOSE(2) {
                  mea_log_printf("%s (%s) : a HUELIGH, HUEGROUP or HUESCENE parameter is mandatory and only one ... (device %s skipped)", WARNING_STR, __func__, name);
               }
               continue;
            }
...
//            stopped=0;
      ...
      DEBUG_SECTION2(DEBUGFLAGON) mea_log_printf("%s (%s) : %s, fin après %d itération\n", DEBUG_STR, __func__, start_stop_params->i004->name, 100-counter);
      
      free(start_stop_params->i004->thread);
      start_stop_params->i004->thread=NULL;
   }
   
   mea_notify_printf('S', "%s %s", start_stop_params->i004->name, stopped_successfully_str);
...
int start_interface_type_004(int my_id, void *data, char *errmsg, int l_errmsg)
{
   ...
   if(start_stop_params->i004->loaded!=1) // les données sont elles déjà chargées ?
   {
      ...
      if(ret<0)
      {
         ...
         mea_notify_printf('E', "%s can't be launched - can't load lights.", start_stop_params->i004->name);
         return -1;
      }
   }
   ...
   if(start_stop_params->i004->loaded==1)
   {
      ...
      if(ret==-1)
      {
         ...
         mea_notify_printf('E', "%s can't be launched - unknow interface device (%s).", start_stop_params->i004->name, start_stop_params->i004->dev);
