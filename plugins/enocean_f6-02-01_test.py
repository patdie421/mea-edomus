...
def testfn():
   ...
   # type de packet = 5
   request[4] = 0x05
   crc8h = mea.enoceanCRC(crc8h, request[4])
   ...
   (rep, nerr, l_response)=mea.sendEnoceanPacketAndWaitResp(data["ID_ENOCEAN"], request, response)
   verbose(1,"TEST : rep = ",rep, ", nerr = ",nerr,", l_response = ",l_response)
   for i in range(0,l_response-1) :
      verbose(1,i,":",response[i])
      ...
      if packet[6]==0xF6:
         ...
         if t21 == 1:
            if nu == 1:
               ...
               if(button1Num==0 and button2Num==2):
                  testfn()
